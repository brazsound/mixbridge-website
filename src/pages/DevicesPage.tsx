...
export function DevicesPage() {
  ...
  useEffect(() => {
    if (!session?.access_token || !API_URL) {
      setLoading(false);
      if (!API_URL) {
        setError('License server URL is not configured for this site build.');
      }
      return;
    }
    ...
    fetch(`${API_URL.replace(/\/$/, '')}/api/web/list-activations`, {
      ...
      .then(async (res) => {
        ...
        try {
          ...
          setError('Invalid response from the license server.');
          return;
        }
        if (!res.ok) {
          setError(data.error ?? `Could not load devices (${res.status}). Try signing out and back in.`);
          return;
        }
      ...
      .catch(() =>
        setError(
          'Could not reach the license server. Check your connection or try again in a moment.'
        )
      )
